...
ap.star_photons = 1e9#1e10#0.5e6# 1e9
...
# tp.aber_vals['a'] = [4.0e-14, 2.0e-14]
...
num_exp = 200#100#2000#1000#50#50#1000
...
ap.contrast = [10**-5.5,10**-5.75,10**-5.7,10**-5.7,10**-6.0]  # [0.1,0.1]
ap.lods = [[2.64,-2.16],[-3.6,3.0],[-3.52,5.76],[-6,-6],[1.5,1.5]]#[6,-4.5],
...
if __name__ == '__main__':
    ...
    if no_sauce:
        ...
        iop.hyperFile = iop.datadir + 'HR8799_SPHERE%inosauce_tar%i.pkl' % (num_exp, np.log10(ap.star_photons))
        simple_hypercube_1 = read.get_integ_hypercube(plot=False)[:, :]  # /ap.numframes
        ap.startframe = ap.numframes
        ap.companion = False
        iop.hyperFile = iop.datadir + 'HR8799_SPHERE%inosauce_ref%i.pkl' % (num_exp, np.log10(ap.star_photons))
        ...
        if os.path.isfile(fname):
            ...
            # ap.star_photons = 1e8
            lod = 10
            psf_template = Analysis.phot.get_unoccult_psf(hyperFile='/IntHyperUnOccult3.pkl', plot=False, numframes=1)
            ...
            # # maps.append(method_out[1])
            #
            method_out = RDI_DSI_SDI(simple_hypercube_1, simple_hypercube_2, psf_template)
            plotdata.append(method_out[0])
            maps.append(method_out[1])
        ...
        ap.companion = True
        iop.hyperFile = iop.datadir + 'HR8799_SPHERE%isauce_tar%i.pkl' % (num_exp, np.log10(ap.star_photons))
        simple_hypercube_1 = read.get_integ_hypercube(plot=False)[:, :]  # /ap.numframes
        ap.startframe = ap.numframes
        ap.companion = False
        iop.hyperFile = iop.datadir + 'HR8799_SPHERE%isauce_ref%i.pkl' % (num_exp, np.log10(ap.star_photons))
        simple_hypercube_2 = read.get_integ_hypercube(plot=False)[:, :]  # /ap.numframes

        maps = quick_processing(simple_hypercube_1, simple_hypercube_2, plot=True)

        compare_images(maps, logAmp=True, vmin = 0.1, vmax= 10,#vmins = [0.1,0.1,0.1,0.1], vmaxs = [100,100,0.5,0.5],
                     annos=['Exp.', 'RDI', 'RDI DSI', 'RDI DSI SDI'])#,
                     #titles=[r'  $I / I^{*}$', r'  $I / I^{*}$', r'  $I_L / I^{*}$', r'  $I_L / I^{*}$'])
        # quicklook_im(maps[0], logAmp=True, vmin = 0.01, vmax = 1)
        plt.show()
