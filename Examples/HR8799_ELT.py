...
tp.diam = 39.0  # telescope diameter in meters
# tp.ao_act = 44
tp.ao_act = 44#50
tp.grid_size=1280#256
mp.array_size = np.array([1000,1000])#np.array([257,257])#
...
if __name__ == '__main__':
    ...
    if no_sauce:
        ...
        ap.companion = False
        iop.hyperFile = iop.datadir + 'HR8799_SPHERE%inosauce_ref_newaber%iPs%i.pkl' % (num_exp, np.log10(ap.star_photons), mp.array_size[0])
        ...
        if os.path.isfile(fname):
            ...
            # ap.star_photons = 1e8
            lod = 10
            psf_template = Analysis.phot.get_unoccult_psf(hyperFile='/RefPSF_wLyotStopPs%i.pkl' % mp.array_size[0], plot=False, numframes=1)
        ...
        compare_images(maps, logAmp=True, vmin = 0.001, vmax= 1.,#vmins = [0.1,0.1,0.1,0.1], vmaxs = [100,100,0.5,0.5],
