    ...
    if no_sauce:
        ...
        if os.path.isfile(fname):
            ...
            for pix_yield in np.arange(1,10)*0.1:#[0.4,0.9,1]:
                for R in [30]:#[10,30,50]:
                    ...
                    iop.hyperFile = iop.datadir + 'HR8799_SPHERE_w%i_%inosauce_tar_%ipy%.2fR%i.pkl' % (tp.w_bins, num_exp, np.log10(ap.star_photons), pix_yield, R)
                    simple_hypercube_1 = read.get_integ_hypercube(plot=False)[:, :]  # /ap.numframes
                    ap.startframe = ap.numframes + 45#20
                    ap.companion = False
                    iop.hyperFile = iop.datadir + 'HR8799_SPHERE_w%i_%inosauce_ref_%ipy%.2fR%i.pkl' % (tp.w_bins, num_exp, np.log10(ap.star_photons), pix_yield, R)
                    ...
                    # ap.star_photons = 1e8
                    lod = 10
                    # psf_template = Analysis.phot.get_unoccult_psf(hyperFile='/RefPSF_wLyotStop.py%.2fR%i.pkl' % ( pix_yield, R), plot=True, numframes=1)
        ...
        for pix_yield in [0.4, 0.9, 1]:
            for R in [10, 30, 50]:
                ...
                ap.companion = True
                iop.hyperFile = iop.datadir + 'HR8799_SPHERE%isauce_tar_%ipy%.2fR%i.pkl' % (num_exp, np.log10(ap.star_photons), pix_yield, R)
                dprint(iop.aberdir)
                simple_hypercube_1 = read.get_integ_hypercube(plot=False)[:, :]  # /ap.numframes
                ap.startframe = ap.numframes + 20
                ap.companion = False
                iop.hyperFile = iop.datadir + 'HR8799_SPHERE%isauce_ref_%ipy%.2fR%i.pkl' % (num_exp, np.log10(ap.star_photons), pix_yield, R)
                simple_hypercube_2 = read.get_integ_hypercube(plot=False)[:, :]  # /ap.numframes
