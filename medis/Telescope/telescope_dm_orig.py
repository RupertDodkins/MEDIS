...
def generate_maps2():
    ...
    aber_cube[0] = proper.prop_psd_errormap(wfo, rms_error, c_freq, high_power,
    ...
    for a in range(1,ap.numframes):
        ...
        aber_cube[a] = proper.prop_psd_errormap(wfo, rms_error, c_freq, high_power,
                             MAP="prim_map", PHASE_HISTORY = phase)
...
def add_aber(wfo,f_lens,aber_params,aber_vals, step=0,Loc='CPA'):
    ...
    if aber_params['Phase']:
        for surf in range(aber_params['n_surfs']):
            ...
            # print c_freq
            high_power = np.random.normal(aber_vals['c'][0],aber_vals['c'][1])  # high frewquency falloff (r^-high_power)
            # print high_power
            # quicklook_wf(wfo)
            if aber_params['OOPP']:
            ...
            #     quicklook_wf(wfo)
            prim_map = proper.prop_psd_errormap(wfo, rms_error, c_freq, high_power, FILE=filename, TPF=True)
            if aber_params['OOPP']:
                proper.prop_propagate(wfo,f_lens+f_lens*(1-1./aber_params['OOPP'][surf]))
                proper.prop_lens(wfo, f_lens, "OOPP")
                # quicklook_wf(wfo)
    ...
    if aber_params['Amp']:
        ...
        for surf in range(aber_params['n_surfs']):
            ...
            high_power = np.random.normal(aber_vals['c'][0],aber_vals['c'][1])  # high frewquency falloff (r^-high_power)
            # print high_power
            prim_map = proper.prop_psd_errormap(wfo, rms_error, c_freq, high_power, FILE=filename, AMPLITUDE=1.0)
            # print 'amp'

            # quicklook_wf(wfo)
            # quicklook_im(prim_map, logAmp=False, colormap="jet", show=True, axis=None, title='nm')
...
def add_atmos(wfo, f_lens, w, atmos_map, correction=False):
    ...
    try:
        ...
        obj_map = proper.prop_errormap(wfo, atmos_map, MULTIPLY = 1.0, WAVEFRONT=True, MAP = "obj_map", SAMPLING=tp.samp)# )##FILE='telescope_objtest.fits'
        ...
        obj_map = proper.prop_errormap(wfo, atmos_map, MULTIPLY=1.0, WAVEFRONT=True, MAP = "obj_map", SAMPLING=tp.samp)
...
def add_spiders(wfo, diam):
    # print 'Including Spiders'
    proper.prop_circular_obscuration(wfo, (diam/3)/2.5)
    proper.prop_rectangular_obscuration(wfo, 0.05*diam, diam*1.3, ROTATION=10)
    proper.prop_rectangular_obscuration(wfo, diam*1.3, 0.05*diam, ROTATION=10)
...
def quick_wfs(wfo, iter, r0):
    ...
    # quicklook_im(CPA_map, logAmp=False)
    sigma = [1, 1]
    CPA_map = scipy.ndimage.filters.gaussian_filter(unwrap_phase(proper.prop_get_phase(wfo)), sigma,
                                                             mode='constant')
    ...
    if tp.piston_error:
        ...
        # debprint(var)
        CPA_map = CPA_map + np.random.normal(0,var,(CPA_map.shape[0],CPA_map.shape[1]))
