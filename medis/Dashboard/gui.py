...
tp.save_locs = np.array([['add_spiders', 'phase'], ['quick_ao', 'phase'], ['prop_mid_optics', 'amp']])
...
class MatplotlibWidget(QtWidgets.QWidget):
    def __init__(self, parent=None, nrows=3, ncols=2):
        ...
        for c in range(self.ncols):
            self.axes[c, 0].text(0.05, 0.075, tp.save_locs[c,0], transform=self.axes[c, 0].transAxes, fontweight='bold', color='w',
...
class ThreadSample(QtCore.QThread):
    ...
    def run(self):
        for t in range(10):
            ...
            # tp.save_locs = [['add_spiders', 'phase']]
            ...
            phase_ind = tp.save_locs[:, 1] == 'phase'
            amp_ind = tp.save_locs[:, 1] == 'amp'
...
class MyWindow(QtWidgets.QWidget):
    ...
    def on_threadSample_newSample(self, gui_images):
        ...
        amp_ind = tp.save_locs[:, 1] == 'amp'
        norm = np.array([None for _ in range(len(tp.save_locs))])
        vmin = np.array([None for _ in range(len(tp.save_locs))])
        vmax = np.array([None for _ in range(len(tp.save_locs))])
