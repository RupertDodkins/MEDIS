...
if __name__ == "__main__":
    ...
    #im = pharo.take_src_return_imagedata(exptime=2)#works, tested
    ...
    #ax3.set_title('Default DM map')
    #ax4.set_title('Map perturbation')
    #w1 = ax1.imshow(pre.histeq(im), origin='lower', interpolation = 'nearest')
    #ax1.set_xlim((min(vertsx), max(vertsx)))
    #ax1.set_ylim((min(vertsy), max(vertsy)))

    #w2 = ax2.imshow(pre.histeq(controlregion*im), origin='lower', interpolation = 'nearest')
    ...
    ##thresh = detect_speckles.detect_speckles(im, configfile = configfilename)
    #speckleobjects = [speckle(im, contour, im_params) for contour in speckleslist]
    #for i in range(len(speckleslist)):
    #    w2.set_data(pre.histeq(controlregion*im))
    #    w2.autoscale()
    #    plt.draw()
    #    plt.pause(0.1) 
    #    zeroim = im*0 
    #    cv2.drawContours(zeroim, speckleslist, i, 1, -1);
    #    w2.set_data((controlregion*(zeroim)*im))
    #    w2.autoscale()
    #    plt.draw()
    #    plt.pause(0.1) 
#for i in range(len(speckleslist)):
    #    zeroim = im*0    
    #    cv2.drawContours(zeroim, speckleslist[i], -1,100, 1)
    #    ipdb.set_trace()
    #    w2.set_data((zeroim))
    #    plt.draw()
