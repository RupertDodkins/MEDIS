...
# iop.update(new_name='HCIdemo_onaxisdim/')
...
def get_form_photons(fields, comps=True):
    ...
    with open(iop.form_photons, 'wb') as handle:
        pickle.dump((photons, stackcube, stem), handle, protocol=pickle.HIGHEST_PROTOCOL)

    return photons, stackcube, stem

def make_figure6(comps=True):
    ...
    iop.form_photons = iop.form_photons[:-4] +'_R'
    iop.device_params = iop.device_params[:-4] + '_R'
    tp.pix_shift = [[0, 0]]
    ap.numframes = 2
    if __name__ == '__main__':
        ...
        for R in Rs:
            mp.R_mean = R
            iop.form_photons = iop.form_photons.split('_R')[0] + f'_R={mp.R_mean}_comps={comps}.pkl'
            iop.device_params = iop.device_params.split('_R')[0] + f'_R={mp.R_mean}_comps={comps}.pkl'
            dprint(iop.form_photons)
            if os.path.exists(iop.form_photons):
                dprint(f'Formatted photon already exists at {iop.form_photons}')
                with open(iop.form_photons, 'rb') as handle:
                    photons, stackcube, stem = pickle.load(handle)

            else:
                photons, stackcube, stem = get_form_photons(fields, comps=comps)
        ...
        if not comps:
            ...
            axes[2].legend([str(R) for R in Rs])
