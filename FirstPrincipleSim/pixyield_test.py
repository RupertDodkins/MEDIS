...
iop.update(new_name='pixyield_test1/')
...
def make_figure(comps=True):
    ...
    yields = [0.4, 0.7, 1]
    if not os.path.exists(iop.device_params.split('_yield')[0] + f'_yield={yields[0]}.pkl'):
        adapt_dp_master(dp_master, yields)
    ...
    iop.form_photons = iop.form_photons[:-4] +'_yield'
    ...
    for y in yields:
        mp.pix_yield = y
        iop.form_photons = iop.form_photons.split('_yield')[0] + f'_yield={y}_comps={comps}.pkl'
    ...
    if not comps:
        ...
        axes[2].legend([str(y) for y in yields])

    view_datacube(maps, logAmp=True, vmin=-1e-7, vmax=1e-7)

# make_figure()

def adapt_dp_master(dp_master, pix_yields):
    ...
    iop.device_params = iop.device_params[:-4] + '_yield'
    bad_inds = get_bad_inds(pix_yields)
    for y, bad_ind in zip(pix_yields, bad_inds):
        mp.pix_yield = y
        dp.QE_map = add_bad_pix(dp.QE_map_all, bad_ind)
        iop.device_params = iop.device_params.split('_yield')[0] + f'_yield={y}.pkl'
