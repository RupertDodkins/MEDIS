; $Id: ssc_gui.pro, Soft.Pack.CAOS v 7.0 2016/05/19 marcel.carbillet $
;+
; NAME:
;    ssc_gui
;
; PURPOSE:
;    ssc_gui generates the Graphical User Interface (GUI) for
;    setting the parameters of the State-Space Control (SSC) module.
;    a parameter file called ssc_nnnnn.sav is created, where nnnnn
...
;    (see ssc.pro's header --or file caos_help.html-- for details
...
;
; CALLING SEQUENCE:
;    error = ssc_gui(n_module, $
;                    proj_name )
; 
; INPUTS:
;    n_module : number associated to the intance of the SSC module
...
;    routine written: 23 february 2012,
...
;;;;;;;;;;;;;;;;;;;
;
pro ssc_gui_event, event

; ssc_gui error management block
...
case uvalue of
   ...
   'help' : online_help, book=(ssc_info()).help, /FULL_PATH
   ...
   'restore': begin
      ...
      widget_control, state.id.dir,        SET_VALUE=par.dir
...
; build the widget id structure where all the needed (in ssc_gui_event)
...
widget_control, root_base_id, SET_UVALUE=state
   ...
   par_base_id = widget_base(state.id.par_base, /ROW)
      ...
      state.id.dir = cw_filename(par_base_id,                                               $
TITLE="directory where the command law files K_A.fits,K_B.fits,K_C.fits,K_D.fits are saved",$
                                         VALUE=state.par.dir,                               $
                                         UVALUE="dir",                                      $
                                         /ALL_EVENTS                                        )
...
; launch xmanager
xmanager, 'ssc_gui', root_base_id, GROUP_LEADER=group
