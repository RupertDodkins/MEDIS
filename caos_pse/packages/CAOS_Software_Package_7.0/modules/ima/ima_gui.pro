; $Id: ima_gui.pro,v 7.0 2016/05/19 marcel.carbillet$
;+
; NAME:
;    ima_gui
;
; PURPOSE:
;    ima_gui generates the Graphical User Interface (GUI) for setting the
;    parameters of the IMage Adding (IMA) module.  A parameter file
;    called ima_yyyyy.sav is created, where yyyyy is the number n_module
...
;
; CALLING SEQUENCE:
;    error = ima_gui(n_module, proj_name)
;
; INPUTS:
;    n_module  : integer scalar. Number associated to the intance
;                of the IMA module. n_module > 0.
...
PRO ima_gui_event, event
   ...
   CASE uvalue OF
      ...
      'help' : online_help, book=(ima_info()).help, /FULL_PATH
...
FUNCTION ima_gui, n_module, proj_name, GROUP_LEADER=group
...
dummy  = WIDGET_LABEL(base1,VALUE="Weight assigned to bottom image",/FRAME)
...
dummy  = WIDGET_LABEL(base1,VALUE="Weight assigned to top image",/FRAME)
...
XMANAGER, 'ima_gui', base, GROUP_LEADER=group
