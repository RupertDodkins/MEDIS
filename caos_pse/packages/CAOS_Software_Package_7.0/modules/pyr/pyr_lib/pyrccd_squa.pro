; $Id pyrccd_squa.pro,v 7.0 2016/05/19 marcel.carbillet $
;+
; NAME:
;   pyrccd_squa.pro
;
...
;-
; PHASE MASK + SQUARE MODULATION ALGORITHM 
function pyrccd_squa,size,dim,wave,l,px,m,pup,fov,pyrcc,iii
...
pyrcc=shift(abs(fft(shift(exp(complex(0.,-1.*m))*imc,size/2,size/2),-1))^2.,size/2,size/2)
...
pyrcc=pyrcc+shift(abs(fft(shift(exp(complex(0.,-1.*m))*imcs,size/2,size/2),-1))^2.,size/2,size/2)
