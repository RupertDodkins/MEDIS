; $Id pyrccd_fftwnd_knife_squa.pro,v 7.0 2016/05/19 marcel.carbillet $
;+
; NAME:
;   pyrccd_fftwnd_knife_squa.pro
;
; PURPOSE:
;   ...
;
...
;-
; TRANSMISSION MASK + SQUARE MODULATION ALGORITHM +FFTW

function pyrccd_fftwnd_knife_squa,size,dim,wave,l,px,m,pup,fov,pyrcc,iii
...
pyrcc(*,*,k)=shift(abs(fftwnd(shift(m(*,*,k)*imc*mask_fov,size/2,size/2),-1))^2.,size/2,size/2)
...
pyrcc(*,*,k)=pyrcc(*,*,k)+shift(abs(fftwnd(shift(m(*,*,k)*imcs*mask_fov,size/2,size/2),-1))^2.,size/2,size/2)
