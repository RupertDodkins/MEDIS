; $Id pyrccd_fftwnd_knife_circ.pro,v 7.0 2016/05/19 marcel.carbillet $
;+
; NAME:
;   pyrccd_fftwnd_knife_circ.pro
;
; PURPOSE:
;   ...
;
...
;-
; TRANSMISSION MASK + CIRCULAR MODULATION ALGORITHM + FFTW 
function pyrccd_fftwnd_knife_circ,size,dim,wave,l,px,m,pup,fov,pyrcc,iii
...
pyrcc(*,*,k)=shift(abs(fftwnd(shift(m(*,*,k)*imc,size/2,size/2),-1))^2.,size/2,size/2)
...
pyrcc(*,*,k)=pyrcc(*,*,k)+shift(abs(fftwnd(shift(m(*,*,k)*imcs,size/2,size/2),-1))^2.,size/2,size/2)
