; $Id: n_phot.pro,v 5.1 2007/05/04 marcel.carbillet $
;+
; NAME:
;    n_phot
;
...
;                    : april 2007,
;                     Arthur Vigan (LAM) [arthur.vigan@oamp.fr]:
;                    -corrected a bug when the given lambda falled
;                     between two defined wide bands.
;
;-
;
function n_phot, mag,             $
...
if (n_elements(band)) then begin
    ...
    band=''
    for i=0,n_elements(lambda_tab)-1 do begin
        if (abs(lambda_tab[i]-lambda) le width_tab[i]/2.) then begin
            dummy = i
            band = band_tab[i]            ; band
        endif
    endfor

    if (band eq '') then begin            ; we are in a case of a narrow band
                                          ; between two defined wide bands
        lambda_min=lambda_tab-width_tab/2 ; min lambda of the wide bands
        lambda_max=lambda_tab+width_tab/2 ; max lambda of the wide bands
        idx_min=where(abs(lambda-lambda_min) $
                      eq min(abs(lambda-lambda_min)))
                                          ; index of the closest lambda in
                                          ; lambda_min
        idx_max=where(abs(lambda-lambda_max) $
                      eq min(abs(lambda-lambda_max)))
                                          ; index of the closest lambda in
                                          ; lambda_max
                                          ; find the index of the closest band
        if (abs(lambda-lambda_min[idx_min]) le $
            abs(lambda-lambda_max[idx_max])) then begin
            dummy = idx_min
            band = band_tab[idx_min]      ; band
        endif else begin
            dummy = idx_max
            band = band_tab[idx_max]      ; band
        endelse
    endif
