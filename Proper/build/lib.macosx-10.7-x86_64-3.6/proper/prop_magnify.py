...
def prop_magnify(image_in, mag0, size_out0 = 0, **kwargs):
    ...
    if np.ndim(image_in) != 2:
        print("PROP_MAGNIFY: Input image is not 2-D. Returning 0.")
        return 0
    ...
    if ("QUICK" in kwargs and kwargs["QUICK"]):
        ...
        if proper.use_szoom_c:
            ...
            if is_complex:
                ...
                image_out_real = np.empty((size_out,size_out), dtype = np.double)
                image_out_imag = np.empty((size_out,size_out), dtype = np.double)

                libszoom_c.prop_szoom_c(image_in_real.T.ctypes.data_as(ct.c_void_p),
                            ct.c_int(image_in_real.T.shape[0]),
                            image_out_real.ctypes.data_as(ct.c_void_p),
                            ct.c_int(size_out), ct.c_double(mag)
                           )
                libszoom_c.prop_szoom_c(image_in_imag.T.ctypes.data_as(ct.c_void_p),
                            ct.c_int(image_in_imag.T.shape[0]),
                            image_out_imag.ctypes.data_as(ct.c_void_p),
                            ct.c_int(size_out), ct.c_double(mag)
                           )
